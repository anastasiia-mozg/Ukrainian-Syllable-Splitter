...
dict_grapheme = {
            ...
            r"((?<=\bі)|(?<=\bне)|(?<=\bпо)|(?<=\bнапів)|(?<=\b))[шс](?=[счшщ])": r"з", #bug трансцендентний - транЗцендентний, for that reason ||ц|| is deleted from (?=[счшщ])
            "(?<=дво|рьо)г(?=[бдзжгкт])": "х",
            "(?<=я)д(?=де)": "т",
            "(?<=ші)[зс](?=[дснц])": "ст"}
...
#            if re.search(pattern_p2g, grapheme): 
#                print(pattern_p2g, subst_p2g)
